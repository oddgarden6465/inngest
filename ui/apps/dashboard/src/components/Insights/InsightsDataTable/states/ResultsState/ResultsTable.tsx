...
export function ResultsTable() {
  ...
  const { openCellDetail, closeCellDetail, selectedCellCoords } =
    useCellDetailContext();
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!selectedCellCoords || !data) return;

      // Build an ordered list of column names so we can index into it
      const colNames = data.columns.map((c) => c.name);
      const colIndex = colNames.indexOf(selectedCellCoords.columnId);
      if (colIndex === -1) return;

      let nextRow = selectedCellCoords.rowIndex;
  ...
  }, [selectedCellCoords, data, openCellDetail, closeCellDetail]);
